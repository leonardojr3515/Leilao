...
public class Leilao implements Serializable {
    ...
    public void propoe(Lance lance){
        ...
        if (defineMaiorEMenorLanceParoOPrimeiroLance(valorLance)) return;
        Collections.sort(lances);
        calculaMaiorLance(valorLance);
    }

    private boolean defineMaiorEMenorLanceParoOPrimeiroLance(double valorLance) {
        if(lances.size() == 1){
    ...
    private boolean usuarioDeuCincoLances(Usuario usuarioNovo) {
        ...
        for (Lance l : lances){
            Usuario usuarioExiste = l.getUsuario();
            if (usuarioExiste.equals(usuarioNovo)){
                lancesDoUsuario++;
